...
{
	if (sys_sync_done == 0) {
		unsigned clk_rate, pclk_rate;
		pr_info("sys_sync time is too long(more than 3 seconds)");
		/* for SD blocking issue debug */
		clk_rate = pc_clk_get_rate(SDC4_CLK);
		pclk_rate = pc_clk_get_rate(SDC4_PCLK);
		pr_info("\n### SD card bloking issue debug ###\n");
		pr_info("\tClock status: clk = %d, pclk = %d\n",
				clk_rate, pclk_rate);
